...
#### デフォルト設定
Ghost Text機能は、**デフォルトで有効**になっています。MIDIエディタを開くと、自動的にAI予測が開始されます。

**初期設定（2025-11-02バグ修正後）**:
- **AIモデル**: `musicRnn` (Magentaモデル) - 音楽理論に基づいた高品質な予測
- **Ghost Text有効化**: ON（デフォルト）
- **予測表示**: 自動表示
- **重要**: useGhostText.js:36で `currentModel` のデフォルト値が `'musicRnn'` に設定されています

#### カスタマイズ方法
必要に応じて、以下の手順で設定を変更できます:
...
4. **AIモデルを選択**: 使用するAIモデルを選択
   - **MusicRNN (デフォルト・推奨)**: Google Magentaの音楽生成モデル、音楽理論に基づいた高品質な予測
   - **MelodyRNN**: メロディ専用の予測エンジン
   - **MusicVAE**: より創造的な予測パターン
   - **Fallback**: シンプルな予測エンジン
...
#### AIモデル選択
- **MusicRNN (デフォルト)**: Google Magentaの音楽生成モデル（自動予測・推奨）
- **MelodyRNN**: メロディ専用の予測エンジン
- **MusicVAE**: より創造的な予測パターン
- **Fallback**: シンプルな予測エンジン
...
| バージョン | 日付 | 変更内容 |
|-----------|------|----------|
| 1.1.0 | 2025-11-02 | デフォルトモデル設定の明確化・バグ修正内容の反映 |
...
**最終バグ修正**: 2025年11月2日 (Issue #136) - Ghost Textボタン動作不良問題を修正
