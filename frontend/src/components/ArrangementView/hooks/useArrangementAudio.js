...
export const useArrangementAudio = (tracks, trackStates, currentTime, metronomeEnabled, bpm, totalDuration, lastClickPosition, pixelsPerSecond, trackVolume = {}, trackMuted = {}, masterVolume = 100) => {
  ...
  const ensureAudioEngineReady = useCallback(async () => {
    if (!window.unifiedAudioSystem) {
      console.error('🎵 [ArrangementAudio] Unified audio system not available');
      return false;
    }

    // 初期化されていない場合は初期化を実行
    if (!window.unifiedAudioSystem.isInitialized) {
      console.log('🎵 [ArrangementAudio] Unified audio system not ready, initializing...');
      const success = await window.unifiedAudioSystem.initialize();
      if (!success) {
        console.error('❌ [ArrangementAudio] Failed to initialize unified audio system');
        return false;
      }
    }
    // 初期化済みでもAudioContextがsuspended状態の場合は再開を試みる
    if (window.unifiedAudioSystem.audioContext &&
        window.unifiedAudioSystem.audioContext.state === 'suspended') {
      console.log('🎵 [ArrangementAudio] AudioContext is suspended, attempting to resume...');
      try {
        await window.unifiedAudioSystem.audioContext.resume();
        console.log('✅ [ArrangementAudio] AudioContext resumed:', window.unifiedAudioSystem.audioContext.state);
      } catch (error) {
        console.warn('⚠️ [ArrangementAudio] Failed to resume AudioContext:', error);
        // ユーザージェスチャー後なので通常は成功するはずだが、失敗しても続行
      }
    }

    // 最終確認: AudioContextがrunning状態かチェック
    if (window.unifiedAudioSystem.audioContext &&
        window.unifiedAudioSystem.audioContext.state === 'running') {
      console.log('✅ [ArrangementAudio] Audio engine is ready (AudioContext: running)');
      return true;
    } else {
      console.warn('⚠️ [ArrangementAudio] Audio engine initialized but AudioContext is not running:',
        window.unifiedAudioSystem.audioContext?.state);
      // suspended状態でも基本的な準備はできているので続行を許可
      return true;
    }
  }, []);
  ...
  const handlePlay = useCallback(async (setIsPlaying, setCurrentTime, setPlaybackPosition) => {
    ...
    // 音声システムの初期化確認と必要に応じた初期化
    const engineReady = await ensureAudioEngineReady();
    if (!engineReady) {
      console.error('❌ [ArrangementAudio] Failed to initialize audio engine');
      return;
    }
    ...
    if (tracksToPlay.length > 0) {
      ...
      console.log('🎵 [ArrangementAudio] EnhancedSoundFontEngine state:', {
        isInitialized: window.unifiedAudioSystem.isInitialized,
  ...
  return {
    ...
    getInitializationStatus: () => ({
      isInitialized: window.unifiedAudioSystem?.isInitialized || false,
      hasUnifiedAudioSystem: !!window.unifiedAudioSystem,
      audioContextState: window.unifiedAudioSystem?.audioContext?.state,
